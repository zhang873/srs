...
<script type="text/javascript">
    ...
    function myback(){
        document.form.action="{{ URL::to('admin/exemption/student_selection') }}";
    }
    function mysave(){
        document.form.action="{{ URL::to('admin/exemption') }}";
    }
</script>
{{-- Content --}}
@section('content')
   {{-- choose input form --}}
    <form id="form" class="form-horizontal" method="post"  autocomplete="off" action="{{ URL::to('admin/exemption')}}" >
        ...
        <table id="exemption" class="table table-striped table-hover" align="center">
            ...
            @foreach ($exemptions as $exemption)
                <tr>
                    ...
                    <td>{{ $exemption->credit }}<input type="hidden" name="student_id" value={{$exemption->student_id}}>
            ...
            @endforeach
            <tr><td colspan="4" align="right"><input type="hidden"  id= "backurl" name="backurl" value="返回" onsubmit="myback()"></td></tr>
            </thead>
            <tbody>
            </tbody>
        </table>
        ...
        <table id="course_outer" class="table table-striped table-hover" align="center" width="600px">
                ...
                <td class="col-md-2">
                    <select name="exemption_type_id">
                        ...
                        @foreach  ($types as $type)
                            <option value="{{$type->exemption_type_id }}">{{$type->exemption_type_name}}</option>
                        @endforeach
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <td colspan="2" align="center" style=""><input type="submit" id="savedata" name="savedata" value=" {{{ Lang::get('admin/exemption/table.save') }}}" onsubmit="mysave()"> </td>
            </tr>
            </thead>
        </table>
     </form>
