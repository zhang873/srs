...
@section('content')
    <div class="form-group" align="center">
        <h3>
            {{ Lang::get('admin/admissions/title.graduate_apply_record') }}
        </h3>
    </div>
    <table id="selection" class="table table-striped table-hover">
        <thead>
            <tr>
                <th></th>
                <th class="col-md-03">{{{ Lang::get('admin/admissions/table.application_year') }}}</th>
                <th class="col-md-03">{{{ Lang::get('admin/admissions/table.application_semester') }}}</th>
                <th class="col-md-03">{{{ Lang::get('admin/admissions/table.application_graduation') }}}</th>
                <th class="col-md-03">{{{ Lang::get('admin/admissions/table.application_degree') }}}</th>
                <th class="col-md-03">{{{ Lang::get('admin/admissions/table.check_result') }}}<br>(教学点)</th>
                <th class="col-md-03">{{{ Lang::get('admin/admissions/table.digital_registration_number') }}}</th>
                <th class="col-md-03">{{{ Lang::get('admin/admissions/table.isReport') }}}</th>
            </tr>
        </thead>
        <tbody>
        </tbody>
...
@section('scripts')
	<script type="text/javascript">
        ...
        oTable = $('#selection').dataTable( {
            ...
            "bServerSide": true,
            "ajax": {
                "url": "{{ URL::to('admin/admissions/query_graduate_data') }}",
                "data": function ( d ) {
                    d["student_id"] = "{{$id}}";
                }
            },
