	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
            oTable = $('#courses').dataTable( {
		        ...
		        "bServerSide": true,
		        "ajax": {
                    "url": "{{ URL::to('admin/course/establish_school_data') }}",
                    "data": function ( d ) {
            ...
            $("#major_classification").change(function(){
                ...
                if (sVal=='全部')
                {
                    for (var i = 0; i < b_majors.length; i ++) {
                        ...
                        $("#major").append("<option value='"+b_majors[i]+"'>"+b_majors[i]+"</option>");   //为Select追加一个Option(下拉项)alert(arr[i]);

                    }
                    for (var i = 0; i < z_majors.length; i ++) {

                        $("#major").append("<option value='"+z_majors[i]+"'>"+z_majors[i]+"</option>");   //为Select追加一个Option(下拉项)alert(arr[i]);
