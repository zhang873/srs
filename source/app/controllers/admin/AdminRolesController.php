...
class AdminRolesController extends AdminController {
    ...
    public function getCreate()
    {
        ...
        usort($permissions, array($this, 'compareDisplayname'));
    ...
    {
        $roles = Role::select(array('roles.id',  'roles.name', 'roles.id as users', 'roles.created_at'))->where('name', '<>', 'applicant');

        return Datatables::of($roles)
        // ->edit_column('created_at','{{{ Carbon::now()->diffForHumans(Carbon::createFromFormat(\'Y-m-d H\', $test)) }}}')
        ->edit_column('users', '{{{ DB::table(\'assigned_roles\')->where(\'role_id\', \'=\', $id)->count()  }}}')
        
        ->edit_column('name','@if($name == \'admin\')
                            	管理员
                        @elseif($name == \'staff\')
                            	教务员
        				@elseif($name == \'teacher\')
        ...
        ->add_column('actions', '<a href="{{{ URL::to(\'admin/roles/\' . $id . \'/edit\' ) }}}" class="iframe btn btn-xs btn-default">{{{ Lang::get(\'button.edit\') }}}</a>
        						@if($name != \'admin\' && $name != \'teacher\' && $name != \'staff\')
